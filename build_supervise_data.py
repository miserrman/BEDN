...
tf.flags.DEFINE_string('X_input_dir', 'datasets/100a',
                       'X input directory, default: data/apple2orange/trainA')
tf.flags.DEFINE_string('Y_input_dir', 'datasets/100b',
                       'Y input directory, default: data/apple2orange/trainB')
tf.flags.DEFINE_string('X_output_file', 'datasets/mingwen.tfrecords',
                       'X output tfrecords file, default: data/tfrecords/apple.tfrecords')
tf.flags.DEFINE_string('Y_output_file', 'datasets/miwen.tfrecords',
...
def data_reader(input_dir, shuffle=True):
  ...
  file_paths = []
  label_path = []
  base_label_path = "datasets/output/"

  for img_file in scandir(input_dir):
    #TODO jpg
    if img_file.name.endswith('.png') and img_file.is_file():
      file_paths.append(img_file.path)
      label_path.append(os.path.join(base_label_path, img_file.name))
  ...
  if shuffle:
    ...
    label_path = [label_path[i] for i in shuffled_index]
...
def _convert_to_example(file_path, image_buffer, label_buffer):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/decoded_image': _bytes_feature(label_buffer),
...
def data_writer(input_dir, output_file):
  ...
  """
  file_paths, label_paths = data_reader(input_dir)
  ...
  for i in range(len(file_paths)):
    ...
    with tf.gfile.FastGFile(file_path, 'rb') as f:
      image_data = f.read()
    with tf.gfile.FastGFile(label_path, 'rb') as f:
      label_data = f.read()
    example = _convert_to_example(file_path, image_data, label_data)
