...
class Discriminator:
  def __init__(self, name, is_training, ngf=64, norm='instance', use_sigmoid=False):
    self.name = name
  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
      # convolution layers
      C64 = ops.Ck(input, self.ngf, reuse=self.reuse, norm=None,
          is_training=self.is_training, name='C64')             # (?, w/2, h/2, 64)
      C128 = ops.Ck(C64, 2*self.ngf, reuse=self.reuse, norm=self.norm,
          is_training=self.is_training, name='C128')            # (?, w/4, h/4, 128)
      C256 = ops.Ck(C128, 4*self.ngf, reuse=self.reuse, norm=self.norm,
          is_training=self.is_training, name='C256')            # (?, w/8, h/8, 256)
      C512 = ops.Ck\
          (C256, 8*self.ngf,reuse=self.reuse, norm=self.norm,
