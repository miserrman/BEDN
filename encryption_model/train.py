...
tf.flags.DEFINE_string('Z', 'datasets/poison.tfrecords',
                       'backdoor tfrecords file for training, default: data/tfrecords/orange.tfrecords')
...
def train():
    ...
    with graph.as_default():
        ...
        G_loss_backdoor, D_Y_loss_backdoor, fake_z = cycle_gan.backdoor_model()
        backdoor_optimizers = cycle_gan.optimize_backdoor(G_loss_backdoor, D_Y_loss_backdoor)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if number >= 1:
                    ...
                    fake_z_val = sess.run([fake_z])
                    _, G_loss_backdoor_val, D_Y_loss_backdoor_val, summary = (
                        sess.run(
                            [backdoor_optimizers, G_loss_backdoor, D_Y_loss_backdoor, summary_op],
                            feed_dict={cycle_gan.fake_y: fake_z_val[0],
                                       cycle_gan.fake_x: fake_z_val[0]}
                        )
                    )
                    G_loss_val = G_loss_backdoor_val
                    D_Y_loss_val = D_Y_loss_backdoor_val
                    F_loss_val = 0
                    D_X_loss_val = 0
                if step % 100 == 0:
